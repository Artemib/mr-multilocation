<template>
  <div>
    <h3 class="text-lg font-medium mb-2">Поддомены</h3>
    <p class="text-slate-600 mb-4">CRUD будет через REST `mr-ml/v1/subdomains`.</p>
...
async function load() {
  ...
  try {
    items.value = await api.getSubdomains();
  } catch (e) {
...
async function onCreate() {
  ...
  try {
    const created = await api.createSubdomain({ slug: draft.slug, name: draft.name });
...
async function onUpdate(row) {
  try {
    await api.updateSubdomain(row.id, { slug: row.slug, name: row.name });
  } catch (e) {
    alert(String(e.message || e));
  }
}
...
async function onDelete(row) {
  ...
  try {
    await api.deleteSubdomain(row.id);
